...
public class TaskAttemptListenerImpTezDag extends AbstractService implements TezTaskUmbilicalProtocol,
		TaskAttemptListener {
	...
	private ConcurrentHashMap<ContainerId, ContainerInfo> registeredContainers = new ConcurrentHashMap<ContainerId, ContainerInfo>();
		...
		if (containerContext == null || containerContext.getContainerIdentifier() == null) {
			...
			if (!registeredContainers.containsKey(containerId)) {
				...
				if (taskContext.shouldDie()) {
					LOG.info("No more tasks for container with id : " + containerId + ". Asking it to die");
					task = TASK_FOR_INVALID_JVM; // i.e. ask the child to die.
				} else {
					if (taskContext.getTask() == null) {
						if (LOG.isDebugEnabled()) {
							LOG.debug("No task currently assigned to Container with id: " + containerId);
						}
					} else {
						registerTaskAttempt(taskContext.getTask().getTaskAttemptID(), containerId);
						...
						context.getEventHandler().handle(
								new TaskAttemptEventStartedRemotely(taskContext.getTask().getTaskAttemptID(),
										containerId, context.getApplicationACLs()));
	...
	/*
	 * @Override public boolean statusUpdate(TezTaskAttemptID taskAttemptId,
	 * TezTaskStatus taskStatus) throws IOException, InterruptedException { if
	 * (LOG.isDebugEnabled()) { LOG.debug("Status update from: " +
	 * taskAttemptId); } taskHeartbeatHandler.progressing(taskAttemptId);
	 * pingContainerHeartbeatHandler(taskAttemptId); TaskAttemptStatusOld
	 * taskAttemptStatus = new TaskAttemptStatusOld(); taskAttemptStatus.id =
	 * taskAttemptId; // Task sends the updated progress to the TT.
	 * taskAttemptStatus.progress = taskStatus.getProgress(); if
	 * (LOG.isDebugEnabled()) { LOG.debug("Progress of TaskAttempt " +
	 * taskAttemptId + " is : " + taskStatus.getProgress()); }
	 * 
	 * // Task sends the updated state-string to the TT.
	 * taskAttemptStatus.stateString = taskStatus.getStateString();
	 * 
	 * // Set the output-size when map-task finishes. Set by the task itself. //
	 * outputSize is never used. taskAttemptStatus.outputSize =
	 * taskStatus.getLocalOutputSize();
	 * 
	 * // TODO Phase // Task sends the updated phase to the TT.
	 * //taskAttemptStatus.phase =
	 * MRxTypeConverters.toYarn(taskStatus.getPhase());
	 * 
	 * // TODO MRXAM3 - AVoid the 10 layers of convresion. // Counters are
	 * updated by the task. Convert counters into new format as // that is the
	 * primary storage format inside the AM to avoid multiple // conversions and
	 * unnecessary heap usage. taskAttemptStatus.counters =
	 * taskStatus.getCounters();
	 * 
	 * 
	 * // Map Finish time set by the task (map only) // TODO CLEANMRXAM - maybe
	 * differentiate between map / reduce / types if
	 * (taskStatus.getMapFinishTime() != 0) { taskAttemptStatus.mapFinishTime =
	 * taskStatus.getMapFinishTime(); }
	 * 
	 * // Shuffle Finish time set by the task (reduce only). if
	 * (taskStatus.getShuffleFinishTime() != 0) {
	 * taskAttemptStatus.shuffleFinishTime = taskStatus.getShuffleFinishTime();
	 * }
	 * 
	 * // Sort finish time set by the task (reduce only). if
	 * (taskStatus.getSortFinishTime() != 0) { taskAttemptStatus.sortFinishTime
	 * = taskStatus.getSortFinishTime(); }
	 * 
	 * // Not Setting the task state. Used by speculation - will be set in //
	 * TaskAttemptImpl // taskAttemptStatus.taskState = //
	 * TypeConverter.toYarn(taskStatus.getRunState());
	 * 
	 * // set the fetch failures if (taskStatus.getFailedDependencies() != null
	 * && taskStatus.getFailedDependencies().size() > 0) {
	 * LOG.warn("Failed dependencies are not handled at the moment." +
	 * " The job is likely to fail / hang"); taskAttemptStatus.fetchFailedMaps =
	 * new ArrayList<TezTaskAttemptID>(); for (TezTaskAttemptID failedAttemptId
	 * : taskStatus .getFailedDependencies()) {
	 * taskAttemptStatus.fetchFailedMaps.add(failedAttemptId); } }
	 * 
	 * // Task sends the information about the nextRecordRange to the TT
	 * 
	 * // TODO: The following are not needed here, but needed to be set
	 * somewhere // inside AppMaster. // taskStatus.getRunState(); // Set by the
	 * TT/JT. Transform into a state // TODO // taskStatus.getStartTime(); //
	 * Used to be set by the TaskTracker. This // should be set by getTask(). //
	 * taskStatus.getFinishTime(); // Used to be set by TT/JT. Should be set //
	 * when task finishes // // This was used by TT to do counter updates only
	 * once every minute. So // this // // isn't ever changed by the Task
	 * itself. // taskStatus.getIncludeCounters();
	 * 
	 * context.getEventHandler().handle( new
	 * TaskAttemptEventStatusUpdate(taskAttemptStatus.id, taskAttemptStatus));
	 * return true; }
	 */
	...
	public AMContainerTask pullTaskAttemptContext(ContainerId containerId) {
		AMContainerImpl container = (AMContainerImpl) context.getAllContainers().get(containerId);
	...
	public void registerRunningContainer(ContainerId containerId) {
		...
		ContainerInfo oldInfo = registeredContainers.put(containerId, new ContainerInfo(containerId));
		if (oldInfo != null) {
	...
	public void registerTaskAttempt(TezTaskAttemptID attemptId, ContainerId containerId) {
		ContainerInfo containerInfo = registeredContainers.get(containerId);
		if (containerInfo == null) {
			throw new TezUncheckedException("Registering task attempt: " + attemptId + " to unknown container: "
					+ containerId);
		}
		synchronized (containerInfo) {
			if (containerInfo.currentAttemptId != null) {
				throw new TezUncheckedException("Registering task attempt: " + attemptId + " to container: "
						+ containerId + " with existing assignment to: " + containerInfo.currentAttemptId);
			}
			...
			if (containerIdFromMap != null) {
				throw new TezUncheckedException("Registering task attempt: " + attemptId + " to container: "
						+ containerId + " when already assigned to: " + containerIdFromMap);
			}
		}
	}
		...
		synchronized (containerInfo) {
			...
			if (taskAttemptID != null) {
				...
				if (containerInfo.lastRequestId + 1 != requestId) {
					throw new TezException("Container " + containerId + " has invalid request id. Expected: "
							+ containerInfo.lastRequestId + 1 + " and actual: " + requestId);
				}
				...
				List<TezEvent> outEvents = context.getCurrentDAG().getVertex(taskAttemptID.getTaskID().getVertexID())
						.getTask(taskAttemptID.getTaskID())
						.getTaskAttemptTezEvents(taskAttemptID, request.getStartIndex(), request.getMaxEvents());
